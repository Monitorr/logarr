                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.status === "success") {
                            ...
                            $("#response").text("Data directory created successfully!");
                            switchTabs("#users");
                            datadirsuccess();
                        } else {
                            console.log("ERROR: Unable to create data directory");
                            $('#response').addClass('regerror');
                            $("#response").text("Unable to create data directory: " + data.response);
                            datadirerror();
                        }
                    },
                    error: function (data) {
        ...
        $(document).on("submit", "#userform", function (e) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    console.log(data);
                    if (data.status === "success") {
                        //TODO: this should be USER NOT datadir?
                        console.log("success, datadir set to: " + data.response);
                        ...
                        //TODO:  Response is not working
                        //$("#responseuser").text("Unable to create user: " + data.response);
                        $("#responseuser").text("Unable to create user: " + data.status);
            ...
            <div id="datadir" class="stepper-target">
                ...
                <?php
                    if ($authenticator->isDatadirSetup()) {
                        ...
                        echo ' <br> If an additional data directory is created, the current data directory will NOT be altered, however, Logarr will use all default resources from the newly created data directory. <br> All settings and user credentials will be reset to default. </b> <br>';
                        echo '</div>';
                    }
                    else {
                    }
                            ...
                            <table id="registrationtable">
                                ...
                                <tbody id="registrationform">
                                    ...
                                    <tr id="userpassword">
                                        ...
                                        <td><input id='login_input_password_repeat' class='login_input input' type='password' name='user_password_repeat' pattern='.{6,}' fv-not-empty=' This field cannot be empty' fv-advanced='{"regex": "\\s", "regex_reverse": true, "message": "  Value cannot contain spaces"}' fv-valid-func='$( "#registerbtn" ).prop( "disabled", false )' fv-invalid-func='$( "#registerbtn" ).prop( "disabled", true )' required autocomplete='off' placeholder=' Repeat password' title='Repeat password'/><i> Minimum 6 characters </i></td>
                                    </tr>
                                </tbody>
                            </table>
